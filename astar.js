...
function drawGrid() {
	for (let y = 0; y < GRID_HEIGHT; y++) {
		for (let x = 0; x < GRID_WIDTH; x++) {
			...
			ctx.fillRect(x * CELL_WIDTH + CELL_BORDER, y * CELL_HEIGHT + CELL_BORDER, CELL_WIDTH - CELL_BORDER * 2, CELL_HEIGHT - CELL_BORDER * 2);
		}
	}
...
function drawStats(gScore, fScore) {
	...
	for (const node of Array.from(fScore.keys())) {
		...
		ctx.textAlign = 'left';
		ctx.fillText(h === Infinity ? '∞' : h, x * CELL_WIDTH + 2, y * CELL_HEIGHT + 2 + CELL_HEIGHT / 4);

		ctx.textAlign = 'right';
		ctx.fillText(g === Infinity ? '∞' : g, x * CELL_WIDTH + CELL_WIDTH - 2, y * CELL_HEIGHT + 2 + CELL_HEIGHT / 4);

		ctx.textAlign = 'center';
		ctx.font = 'normal 30px monospace';
		ctx.fillText(f === Infinity ? '∞' : f, x * CELL_WIDTH + CELL_WIDTH / 2, y * CELL_HEIGHT + 2 + (CELL_HEIGHT / 4) * 3);
	}
}
...
async function solve() {
	const shortestPath = await graph.astar(`${startNode.x}-${startNode.y}`, `${endNode.x}-${endNode.y}`);
	...
	if (shortestPath) {
		...
		for (const n of shortestPath) {
			...
			ctx.fillRect(node.data.x * CELL_WIDTH + 5, node.data.y * CELL_HEIGHT + 5, CELL_WIDTH - 10, CELL_HEIGHT - 10);
		}
	}
}
